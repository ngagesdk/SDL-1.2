 ...
 *
 * $Header: /cvsroot/scummvm/scummvm/sound/mp3.cpp,v 1.8 2004/01/06 12:45:33 fingolfin Exp $
 *
 */
...
MP3TrackInfo::MP3TrackInfo(File *file) {
	...
	while (1) {
		if (buflen < 8192) {
			...
			bytes = file->read(buffer + buflen, 8192 - buflen);
			if (bytes <= 0) {
				if (bytes == -1) {
