 ...
 *
 * $Header: /cvsroot/scummvm/scummvm/sound/mixer.cpp,v 1.157.2.1 2004/05/02 16:36:05 aquadran Exp $
 *
 */
...
public:
    ...
    const PlayingSoundHandle& handle() const {return *_handle;}

    bool isDrained() const
        {
        return _input->endOfData();
        }
...
public:
	...
	int append(void *sound, uint32 size);
...
int SoundMixer::appendStream(PlayingSoundHandle handle, void *sound, uint32 size) {
	Common::StackLock lock(_mutex);
	...
	ChannelStream *chan;
#if !defined(_WIN32_WCE) && !defined(__PALM_OS__) && !defined(__SYMBIAN32__)
		...
		for (int i = 0; i != NUM_CHANNELS; i++)
			if (_channels[i]) {
				if (_channels[i]->isFinished()) {
                    ...
                    {
					const int read = _channels[i]->mix(buf, len);
                    if(_readObserver != NULL) 
                        _readObserver->channelRead(_channels[i]->handle(), read << 1);
                    }
			}
	}
}
...
int SoundMixer::getChannelVolume(PlayingSoundHandle handle) const {
	Common::StackLock lock(_mutex);
...
bool SoundMixer::isDrained(PlayingSoundHandle handle) const
    {
    if (!handle.isActive())
		return true;
	...
	if (_channels[index])
		return _channels[index]->isDrained();
    return true;
    }
...
int Channel::mix(int16 *data, uint len) {
	...
	if (_input->endOfData())
		...
		return  _converter->flow(*_input, data, len, vol_l, vol_r);
...
int ChannelStream::append(void *data, uint32 len) {
	return ((AppendableAudioStream *)_input)->append((const byte *)data, len);
}
