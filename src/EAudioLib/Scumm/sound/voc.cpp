 ...
 *
 * $Header: /cvsroot/scummvm/scummvm/sound/voc.cpp,v 1.13.2.3 2004/04/11 12:31:22 fingolfin Exp $
 *
 */
...
byte *readVOCFromMemory(byte *ptr, int &size, int &rate, int &loops, int &begin_loop, int &end_loop) {
	...
	assert(memcmp(ptr, "Creative Voice File", 19) == 0);
...
byte *loadVOCFile(File *file, int &size, int &rate) {
	...
	assert(memcmp(&fileHeader, "Creative Voice File", 19) == 0);
