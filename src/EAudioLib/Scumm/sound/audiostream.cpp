 ...
 *
 * $Header: /cvsroot/scummvm/scummvm/sound/audiostream.cpp,v 1.55.2.1 2004/05/02 16:36:04 aquadran Exp $
 *
 */
...
public:
	...
	int append(const byte *data, uint32 len);
...
int AppendableMemoryStream<stereo, is16Bit, isUnsigned>::readBuffer(int16 *buffer, const int numSamples)
    ...
    while(samples < numSamples && ! eosIntern())
        ...
        const int offs = (is16Bit ? 1 : 0);
        const int left = (int)(endMarker - _pos) >> offs;
        const int len = MIN(numSamples - samples, samples + left) << offs;
        memcpy(buffer, _pos, len);
        samples += len;
        buffer += len;
        _pos += len;
        }
...
template<bool stereo, bool is16Bit, bool isUnsigned>
int AppendableMemoryStream<stereo, is16Bit, isUnsigned>::append(const byte *data, uint32 len) {
