...
public:
    ...
    bool operator==(const PlayingSoundHandle& handle) const { return val == handle.val; }
    };

class MChannelReadObserver
    {
    public:
        virtual void channelRead(const PlayingSoundHandle& aHandle, int amount) = 0;
    };
...
private:
    ...
    MChannelReadObserver* _readObserver;
...
public:
    ...
    /** */
    void setReadObserver(MChannelReadObserver* obs) {_readObserver = obs;};
	...
	int appendStream(PlayingSoundHandle handle, void *sound, uint32 size);
    ...
    /**                                             */
    int getChannelVolume(PlayingSoundHandle handle) const; 

    /**                                            */
    bool isDrained(PlayingSoundHandle handle) const;
