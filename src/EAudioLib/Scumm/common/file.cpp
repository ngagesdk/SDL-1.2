...
static Common::String* _mydefaultDirectory = NULL;
...
#if !defined(__GP32__) && !defined(__PALM_OS__) && !defined(__SYMBIAN32__)
...
void File::setDefaultDirectory(const Common::String &directory) {
	if(_mydefaultDirectory==NULL)
	{
		_mydefaultDirectory=new Common::String;
	}
	*_mydefaultDirectory=directory;
//	_mydefaultDirectory = directory;
}
...
bool File::open(const char *filename, const char *directory, AccessMode mode, byte encbyte) {
	...
	if (directory == NULL)
	{
		if(_mydefaultDirectory)
			directory = _mydefaultDirectory->isEmpty() ? "" : _mydefaultDirectory->c_str();
		else
			directory = "";
	}
