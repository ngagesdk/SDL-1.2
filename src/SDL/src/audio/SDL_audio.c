...
#ifdef SAVE_RCSID
static char rcsid =
 "@(#) $Id: SDL_audio.c,v 1.7.2.42 2001/03/21 17:19:56 hercules Exp $";
#endif
...
{
	SDL_AudioDevice *audio = current_audio;
    SDL_TRACE("SDL:SDL_AudioQuit");
	if ( audio ) {
		audio->enabled = 0;
        if ( audio->opened ) {          //did these were in wrong order?
			audio->CloseAudio(audio);   //audion thread must be stopped
		audio->opened = 0;              //before resources can be freed?
                                        //at least Doom get jammed sometimes
                                        //coz thread was not signalled - Markus
        ...
        //audio open was originally here - Markus
		}
